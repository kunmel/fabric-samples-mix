...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS   -c '{"Args":["sgxTx","a/b","true","true","workload1","c"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    ...
    peer chaincode invoke -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS -c '{"Args":["sgxTx","a/b","true","true","workload1","c"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
